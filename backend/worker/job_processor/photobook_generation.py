    ...
    async def process(
        ...
        async with self.db_session_factory.new_session() as db_session:
            async with safe_commit(db_session):
                ...
                logging.info(
                    f"[{self.job_uuid}] [timing] create_many pages: {time.perf_counter() - start:.3f} sec"
                )
                ...
                logging.info(
                    f"[{self.job_uuid}] [timing] create_many pages_assets_rel: {time.perf_counter() - start:.3f} sec"
                )
                ...
                logging.info(
                    f"[{self.job_uuid}] [timing] update photobook: {time.perf_counter() - start:.3f} sec"
                )
